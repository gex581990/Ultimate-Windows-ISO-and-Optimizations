menu(title='Environment' mode='multiple' type='file|dir|back.dir|drive|back.drive|desktop' image=\uE0A0)
{
	...
	item(image=\uE258 keys='SHIFT writable' title='Run Executable File in Sandbox 2' type='file' where=sel.file.ext=='.exe'
    tip='Runs the selected executable file in Windows Sandbox. Hold SHIFT for writable access.'
    cmd-ps=`$wsbConfig='<Configuration><VGPU>Disable</VGPU><Networking>Enable</Networking><MappedFolders>
        <MappedFolder><HostFolder>@sel.parent</HostFolder><SandboxFolder>@dir_app</SandboxFolder><ReadOnly>@if(keys.shift(), 'false', 'true')</ReadOnly></MappedFolder>
        <MappedFolder><HostFolder>@app.dir</HostFolder><SandboxFolder>C:\Shell</SandboxFolder><ReadOnly>@if(keys.shift(), 'false', 'true')</ReadOnly></MappedFolder>
        </MappedFolders><LogonCommand><Command>powershell -ExecutionPolicy Bypass -Command \"Start-Process @fl; Start-Process -FilePath "C:\Shell\Shell.exe"</Command></LogonCommand></Configuration>'; @ps_command` window='hidden')
